...
class RLRobotImageDataset(BaseImageDataset):
    def __init__(self,
            zarr_path, 
            horizon=1,
            pad_before=0,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            zarr_path,
            # keys=['cabinet_pose', 'apple_pose', 'head_camera', 'front_camera', 'left_camera', 'right_camera', 'state', 'action'],
            keys=['head_camera', 'state', 'action',  
                   'reward', 'done', 'next_head_camera',  'next_state']
        )
    ...
    def postprocess(self, samples, device):
        ...
        done = samples['done'].to(device, non_blocking=True)
        return {
            'obs': {
                'head_cam': head_cam, # B, T, 3, H, W
                # 'front_cam': front_cam, # B, T, 3, H, W
