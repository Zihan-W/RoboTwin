...
from diffusion_policy.policy.mlp_agent import MLPAgent
...
class MLPAgentRobotWorkspace(BaseWorkspace):
    include_keys = ['global_step', 'epoch']
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: MLPAgent = hydra.utils.instantiate(cfg.policy)

        self.ema_model: MLPAgent = None
        if cfg.training.use_ema:
            self.ema_model = copy.deepcopy(self.model)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        raw_loss = loss_dict['total_loss']  # TODO:修改loss为自己需要的loss
                        loss = raw_loss / cfg.training.gradient_accumulate_every
                        loss.backward()
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                batch = dataset.postprocess(batch, device)
                                loss = self.model.compute_loss(batch)['total_loss']   ##TODO:修改为自己的loss
                                val_losses.append(loss)
                                if (cfg.training.max_val_steps is not None) \
                                    and batch_idx >= (cfg.training.max_val_steps-1):
                                    break
