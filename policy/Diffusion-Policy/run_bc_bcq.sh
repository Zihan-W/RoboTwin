...
alg_name=robot_bc_bcq
# task choices: See TASK.md
config_name=${alg_name}
addition_info=train
exp_name=${task_name}-robot_bc_bcq-${addition_info}
...
if [ ! -d "./data/${task_name}_${head_camera_type}_${expert_data_num}.zarr" ]; then
    echo "zarr does not exist, run pkl2zarr_rl.py"
    cd ../..
    ...
    if [ ! -d "./data/${task_name}_${head_camera_type}_pkl/episode${expert_data_num_minus_one}" ]; then
        ...
        python script/pkl2zarr_rl.py ${task_name} ${head_camera_type} ${expert_data_num}
...
python bc_bcq.py --config-name=${config_name}.yaml \
                            task.name=${task_name} \
